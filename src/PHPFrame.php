 ...
 * @version    SVN: $Id: PHPFrame.php 1057 2014-06-02 11:51:42Z chrismcband@gmail.com $
...
class PHPFrame
{
    ...
    {
        if (self::isTestMode()) {
            return PHPFrame_Registry_MockSession::getInstance();
        }
    ...
    private static function _loadLanguage()
    {
        ...
        if (defined("PHPFRAME_INSTALL_DIR")) {
            $lang_file = PHPFRAME_INSTALL_DIR.DS."src".DS."lang".DS;
            $lang_email_file = $lang_file.PHPFrame::Config()->get("DEFAULT_LANG")."_email.php";
            $lang_permissions_file = $lang_file.PHPFrame::Config()->get("DEFAULT_LANG")."_permissions.php";
            $lang_file .= PHPFrame::Config()->get("DEFAULT_LANG").".php";
            
            if (file_exists($lang_email_file)) {
                require_once $lang_email_file;
            } else {
                $msg = 'Could not find language file ('.$lang_email_file.')';
                throw new PHPFrame_Exception($msg);
            }
            
            if (file_exists($lang_permissions_file)) {
                require_once $lang_permissions_file;
            } else {
                $msg = 'Could not find language file ('.$lang_permissions_file.')';
                throw new PHPFrame_Exception($msg);
            }
