 ...
 * @version    SVN: $Id: User.php 970 2010-11-24 18:13:11Z chrismcband@gmail.com $
...
class PHPFrame_User
{
    ...
    /**
     * Gets the named parameter for this User, this is an additional parameter
     * which is not a main field for the User and is stored in a serialized 
     * form.
     * 
     * @return mixed the value if the key exists or NULL
     */
    ...
    /**
     * Sets the named parameter to the given value. This parameter is in 
     * additional to the existing user fields. These <i>extra</i> attributes 
     * are persisted in a serialized form.
     * 
    ...
    public function load($id, $exclude='password') 
    {
        ...
        if (!empty($params)){
        	$this->_params = unserialize($params);
        }
    ...
    public function store($force_insert=false) 
    {
        ...
        if (!is_null($this->_row->get('password')) && $this->_passMod) {
        ...
        $this->_row->set('params', serialize($this->_params));
        
        // Invoke row store() method to store row in db
        return $this->_row->store($force_insert);
    }
    ...
    public function setPassMod($modified)
    {
    	$this->_passMod = $modified;
    }
