            ...
            if (isset($component_matches[1]) && !empty($component_matches[1])) {
                ...
                if ($component == 'com_api') {
                    if (isset($action) && $action == 'rooms') {
                        ...
                        preg_match('/^api\/(notes|messages|work|members|users|contacts|contactlists|contentmedia|roomcontentmedia|posts|invites|roominvites|worldinvites|requests|icons|images|stickers|events|contextviews|questions|questionchoices|questionanswers|webhooks)\/(\d+)/', $params, $noteid_matches);
                        if (isset($noteid_matches[2])) {
                            $_REQUEST['id'] = $noteid_matches[2];
                            $_GET['id'] = $noteid_matches[2];
                        } else {
                            preg_match('/^api\/(notes|messages|work|members|users|contacts|contactlists|contentmedia|roomcontentmedia|posts|invites|roominvites|worldinvites|requests|icons|images|stickers|events|contextviews|questions|questionchoices|questionanswers|webhooks)\/([a-zA-Z\/\_]+)/', $params, $subpath_matches);
                        ...
                        preg_match('/^api\/(notes|messages|work|members|users|contacts|contactlists|contentmedia|roomcontentmedia|posts|invites|requests|icons|images|stickers|events|contextviews|questions|questionchoices|questionanswers|webhooks)\/(\d+)\/([a-zA-Z\/]+)/', $params, $path_matches);
