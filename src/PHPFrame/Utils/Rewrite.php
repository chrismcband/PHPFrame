            ...
            if (isset($component_matches[1]) && !empty($component_matches[1])) {
                ...
                if ($component == 'com_api') {
                    if (isset($action) && $action == 'rooms') {
                        ...
                        preg_match('/^api\/(notes|messages|work|members|users|contacts|contactlists|contentmedia|roomcontentmedia|posts|invites|requests|icons|images|stickers|events|contextviews|questions|questionchoices|questionanswers)\/(\d+)\/([a-zA-Z\/]+)/', $params, $path_matches);
