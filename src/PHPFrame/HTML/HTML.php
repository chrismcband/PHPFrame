 ...
 * @version    SVN: $Id: HTML.php 1029 2012-07-18 18:19:22Z chrismcband $
...
//                    })
		...
		$(document).ready(function() {
		    ...
		    $("#dialog_<?php echo $uid; ?>").dialog({
        		...
        		open: function(event) {
        		     ...
        		     $('.ui-dialog-buttonpane').find('button:contains("No")').addClass('cancel');
        		},
		        resizable: false,
		        buttons: {
		            "<?php echo $close_btn;?>" : function() {
		                $(this).dialog('close');
		                $(this).empty();
		            }<?php if ($form) : ?>,
		            "<?php echo $save_btn;?>" : function() {
		    ...
		    $('#dialog_trigger_<?php echo $uid; ?>').click(function(e) {
		        ...
		        $("#dialog_<?php echo $uid; ?>").load($("#dialog_trigger_<?php echo $uid; ?>").attr('href')+"&tmpl=component");
    ...
    public static function confirm($a_class, $title, $msg, $ajax_container='') 
    {
        ...
        $(function() {
            ...
            $("#confirm_dialog_<?php echo $uid; ?>").dialog({
                ...
                open: function(event) {
           		     $('.ui-dialog-buttonpane').find('button:contains("Cancel")').addClass('cancel');
           		     $('.ui-dialog-buttonpane').find('button:contains("Close")').addClass('cancel');
           		     $('.ui-dialog-buttonpane').find('button:contains("Ok")').addClass('confirm_ok_<?php echo $a_class;?>');
           		},
                overlay: {
                    backgroundColor: '#000',
                    opacity: 0.5
            ...
            $("a.<?php echo $a_class; ?>").click(function(e) {
                ...
                $("#confirm_dialog_<?php echo $uid; ?>").html('<?php echo $msg; ?> <br/>' + confirm_title_<?php echo $uid; ?> + '?');
     ...
     * Uses a textarea as the autocomplete field and allows multiple 
     * suggestions to be entered.
     * 
     ...
     * @param	 string	 $field_id		   Optional parameter (default: null), field id to use for the autocomplete 
     * 									   field, use this to ensure autocomplete field is unique, if ommitted 
     * 									   the field_name will be used as the element id
     * @return     void
     * @since    1.0
     */
    public static function autocomplete($field_name, $attribs, $tokens, $matchContains=true, $field_id=null) 
    {
...
//        $document->addStyleSheet('lib/jquery/plugins/autocomplete/jquery.autocomplete.css');
        ...
        for ($i=0; $i<count($tokens); $i++) {
            if ($i>0) $users_string .= ";";
            $users_string .= $tokens[$i]['id']."|".trim($tokens[$i]['name']);
        }
        ?>
        
        <textarea name="<?php echo $field_name; ?>_autocomplete" id="<?php echo $field_id; ?>_autocomplete" <?php echo $attribs; ?>></textarea>
        <input name="<?php echo $field_name; ?>" id="<?php echo $field_id; ?>" type="hidden" />
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $("#<?php echo $field_id; ?>_autocomplete").autocomplete(data, {
                    multiple: true,
                    multipleSeparator: ", ",
                ...
                $("#<?php echo $field_id; ?>_autocomplete").result(function(event, data, formatted) {
                    var hidden = $("#<?php echo $field_id; ?>");
                    hidden.val( (hidden.val() ? hidden.val() + "," : hidden.val()) + data[0]);
                });

                $("#<?php echo $field_id; ?>_autocomplete").keyup(function(event){
                    if (event.which == 8 || event.which == 46) {
                        //backspace or delete was pressed, check autocomplete values
                        //to see if field_id values should change
                        ...
                        for (var i = 0; i < items.length; i++) {
                            ...
                            for (var j = 0; j < data.length; j++) {
                                if (data[j][1] == item) {
                                    field_ids.push(data[j][0]);
                                    break;
                                }
                            }
                        }
                        var hidden = $("#<?php echo $field_id;?>");
                        hidden.val(field_ids.join(","));
                    }
                });
    ...
    /**
     * Function to build input with autocomplete textinput and display it. 
     * Uses a text input as the autocomplete field, only allows for a single 
     * suggestion to be entered.
     * 
     * @param     string    $field_name        The name attribute fot the input tag
     * @param    array    $tokens            An array with the key/value pairs used to build the list of options
     * @param    bool    $matchContains    Optional parameter (default: TRUE). If TRUE search matches inside string, 
     ...
     * @since    1.0
     */
    public static function autocomplete_textinput($field_name, $tokens, $matchContains=true, $showOnClick=false, $field_id=null, $placeholder_text='')
    {
        $document = PHPFrame::Response()->getDocument();
        $document->addScript('lib/jquery/plugins/autocomplete/jquery.autocomplete.pack.js');
        $document->addStyleSheet('lib/jquery/plugins/autocomplete/jquery.autocomplete.css');
        ...
        <input type="text" name="<?php echo $field_name; ?>_autocomplete" id="<?php echo $field_id; ?>_autocomplete"
               <?php if (!empty($placeholder_text)) echo 'placeholder="'.$placeholder_text.'"';?>
        />
        <input name="<?php echo $field_name; ?>" id="<?php echo $field_id; ?>" type="hidden" />
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $("#<?php echo $field_id; ?>_autocomplete").result(function(event, data, formatted) {
                    var hidden = $("#<?php echo $field_id; ?>");
                    hidden.val(data[0]).trigger("change");
                });
